...
#         self.actor_local.eval()
        with torch.no_grad():
            ...
            for id, state in enumerate(states):
                actions[id, :] = self.actor_local(state).cpu().data.numpy()
    ...
    def concat(self, agent_id, actions_new, actions):
        return torch.cat(
           (actions_new, actions[:, 2:]) if agent_id == 0 else (actions[:, :2], actions_new),
           dim=1
        )
    ...
    def learn(self, experiences, gamma, agent_id):
        ...
        actions_next = self.concat(agent_id, actions_next, actions)
        ...
        actions_pred = self.concat(agent_id, actions_pred, actions)
